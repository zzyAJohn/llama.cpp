...
def clean_output(text: str) -> str:
    ...
    # 1. 找到第一个以 "> " 开头的行（提示符 + 用户输入回显）
    start_idx = None
    for i, line in enumerate(lines):
        if line.startswith("> "):
            start_idx = i + 1   # 从下一行开始收集
            break

    # 2. 如果没有找到 "> "，回退到原有关键词过滤（兼容旧版本或不同输出格式）
    if start_idx is None:
        ...
        for line in lines:
            ...
            if any(keyword in line for keyword in [
                ...
                continue
            # 过滤 ASCII 艺术 banner（如有）
            if set(line.strip()) <= set("▄█▀ "):
                continue
            filtered.append(line)
        return "\n".join(filtered).strip()

    # 3. 从 start_idx 开始收集，同时过滤可能残留的退出信息
    result_lines = []
    for line in lines[start_idx:]:
        # 过滤退出信息（通常在最后）
        if any(keyword in line for keyword in ["Exiting...", "Loading model", "available commands"]):
            continue
